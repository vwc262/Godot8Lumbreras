...
func scroll_scene(tipo_nivel: TIPO_NIVEL, idKeySceneToLoad: int):
	var nivel_existente = scenes.has(idKeySceneToLoad)
	current_tipoNivel = tipo_nivel
	current_scene_id = idKeySceneToLoad
	if UIManager.ui_particular != null:
		UIManager.ui_particular.set_fondos_temporales(idKeySceneToLoad)
